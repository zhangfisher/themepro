...
export class AutoButton extends AutoElementBase<AutoButtonProps> {
    ...
    private _handleCheckEvent(_e: MouseEvent) {
        ...
        this.value =
            this.value === checkValues[0] ? checkValues[1] : checkValues[0];
    ...
    private _getTags(): AutoButtonTags {
        ...
        return tags
            ...
            .map((tagArgs) => {
                ...
                if (tag.checkValues.length === 0)
                    tag.checkValues.push(true, false);
                if (tag.checkValues.length === 1)
                    tag.checkValues.push(tag.checkValues[0]);
                return tag;
            });
    }
    ...
    private _renderTag(tag: AutoButtonTag) {
        ...
        const icon = tag.checkable
            ? tag.value === tag.checkValues![0]
                ? icons[0]
                : icons[1]
            : tag.icon;
        ...
        >
            ${when(
                tag.icon,
                () => html`<auto-icon inherit name="${icon!}"> </auto-icon>`
            )}
            ${tag.label}
        </span>`;
    }
    ...
    render() {
        ...
        return html`
            ...
            ${when(
                !this.loading && !isChecked && this.checkPos === "before",
                () => this._renderUnchecked()
            )}
            ${when(
                !this.loading && isChecked && this.checkPos === "before",
                () => this._renderChecked(true)
            )}
            ${when(
                this.icon,
                () =>
                    html`<auto-icon
                        inherit
                        size=${ifDefined(
                            this.vertical || this.shape === "circle"
                                ? "1.5em"
                                : undefined
                        )}
                        name="${this.icon!}"
