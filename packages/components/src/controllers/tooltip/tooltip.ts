...
export class Tooltip {
    ...
    constructor(
        ...
        if (!this.options.className) this.options.className = this.options.dataPrefix
        if (!this.options.cssClass) this.options.cssClass = `${this.options.dataPrefix}-visible`
    ...
    private _setPredictSize(url?: string) {
        if (this.contentElement) {
            ...
            if (w) this.contentElement.style.width = isNumber(w) ? `${w}px` : String(w)
            if (h) this.contentElement.style.height = isNumber(h) ? `${h}px` : String(h)
        }
    }
    ...
    private _getTooltipContent(): string | HTMLElement | undefined | null {
        ...
        if (el instanceof HTMLElement) {
            return el ? removeUnescapedChars((el.cloneNode(true) as HTMLElement).outerHTML) : null
        } else {
    ...
    private _onMouseLeave = (e: any) => {
        ...
        this._mouseLeaveTimer = setTimeout(() => {
            ...
            const hasDeepestTooltipHover = this.controller.tooltips.some((t) => {
                ...
                const tooltipEl = t.el.deref()
                if (!tooltipEl) return false

                // 检查是否是嵌套在当前tooltip内的子tooltip
                const isNestedTooltip = this.container?.contains(tooltipEl)
            ...
            if (!hasDeepestTooltipHover) {
                // 没有更深层级的tooltip被hover，可以隐藏
                this.hide()
            } else {
                // 有更深层级的tooltip被hover，不隐藏
                // 清除定时器
                clearTimeout(this._mouseLeaveTimer)
    ...
    private _addEventListeners(): void {
        ...
        if (this._container) {
            ...
            // 添加容器内部事件监听器，用于支持嵌套 tooltip
            // 将容器内的事件转发到 controller，使其可以处理嵌套的 tooltip 元素
            this._containerEventHandlers = {
                mousemove: (e: MouseEvent) => {
                    // 将 mousemove 事件转发到 controller
                    // 这样 controller 的 _getTooltipElement 就能检测到容器内的 tooltip 元素
                    this.controller.handleContainerEvent?.(e)
                },
                click: (e: Event) => {
                    // 将 click 事件转发到 controller
                    this.controller.handleContainerEvent?.(e)
                },
            }

            // 使用捕获阶段监听，确保事件能被正确转发
            this._container.addEventListener('mousemove', this._containerEventHandlers.mousemove, true)
            this._container.addEventListener('click', this._containerEventHandlers.click, true)
        }
        // 延迟添加事件监听器，避免当前事件触发
        setTimeout(() => {
    ...
    private _removeEventListeners() {
        ...
        if (this._container) {
            ...
            // 移除容器内部事件监听器
            if (this._containerEventHandlers) {
                this._container.removeEventListener('mousemove', this._containerEventHandlers.mousemove, true)
                this._container.removeEventListener('click', this._containerEventHandlers.click, true)
                this._containerEventHandlers = undefined
            }
        }
        const document = this.host.ownerDocument
     ...
     * 更新箭头位置和样式
     */
    private _updateArrowPosition(middlewareData: ComputePositionReturn['middlewareData'], placement?: string): void {
        if (!this.options.arrow || !this._arrowElement || !middlewareData.arrow || !this._container) {
     ...
     */
    private _updatePosition(setupAutoUpdate: boolean = true, async: boolean = true): Promise<void> | void {
    ...
    private _applyCssClass(isShowing: boolean): void {
        ...
        const [showClass, hideClass] = Array.isArray(cssClass) ? cssClass : cssClass.split(',')
        if (isShowing) {
    ...
    hide() {
        ...
        this._hideAnimation = animate(targets, {
            ...
            easing: options.animationEasing || options.animationEasing || 'easeInQuart',
        })
    ...
    destroy() {
        ...
        if (!this.options.cache) {
            setTimeout(() => {
                ...
                if (this.container && this.controller.themeproContainer?.contains(this.container)) {
