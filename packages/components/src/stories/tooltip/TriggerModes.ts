...
export const 鼠标悬停触发: Story = {
  ...
  render: () => html`
    <div style="display: flex; gap: 20px; flex-wrap: wrap; padding: 40px;">
      ...
      <div
        data-tooltip="容器也可以悬停"
        data-tooltip-trigger="mouseover"
        style="padding: 15px; background: #e8f4f8; border-radius: 8px; cursor: pointer;"
      >
        悬停容器
      </div>
    </div>
  ...
  parameters: {
    docs: {
      description: {
        story: "使用 `data-tooltip-trigger=\"mouseover\"` 设置鼠标悬停触发，这是默认的触发方式。",
      },
    },
  },
};
...
export const 混合触发方式: Story = {
  ...
  render: () => html`
    <div style="display: grid; gap: 30px; padding: 40px;">
      <div>
        <h3 style="margin-bottom: 15px;">同一页面中的不同触发方式</h3>
